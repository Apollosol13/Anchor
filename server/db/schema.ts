...
export const verseOfTheDay = pgTable(
  ...
  (table) => [
    unique('verse_of_the_day_date_version_unique').on(table.date, table.version),
...
export const aiChatUsage = pgTable(
  ...
  (table) => [
    unique('ai_chat_usage_user_date_unique').on(table.userId, table.lastResetDate),
...
export const verseLibrary = pgTable(
  ...
  (table) => [
    index('idx_verse_library_theme').on(table.theme),
  ]
);
